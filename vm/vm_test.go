...
func TestConditionals(t *testing.T) {
	tests := []vmTestCase{
		...
		{"if (1 > 2) { 10; }", Null},
		{"if (false) { 10; }", Null},
		{"if ((if (false) { 10; })) { 10; } else { 20; }", 20}, // use Null as expression for condition
	}
	runVmTests(t, tests)
}
...
func TestCallingFunctionsWithBindings(t *testing.T) {
	tests := []vmTestCase{
		...
		{
			input: `let oneAndTwo = fn() { let one = 1; let two = 2; return one + two; };
					oneAndTwo();`,
			expected: 3,
		},
		{
...
func TestCallingFunctionsWithArgumentsAndBindings(t *testing.T) {
	tests := []vmTestCase{
		{
			input: `let identity = fn(a) { return a; };
					identity(4);`,
			expected: 4,
		},
		{
