...
func TestDefine(t *testing.T) {
	...
	if c != expected["c"] {
		t.Errorf("expected c=%+v, got=%+v", expected["a"], a)
	}
	d := firstLocal.Define("d")
	if d != expected["d"] {
		t.Errorf("expected d=%+v, got=%+v", expected["a"], a)
	}
	secondLocal := NewEnclosedSymbolTable(firstLocal)
	e := secondLocal.Define("e")
	if e != expected["e"] {
		t.Errorf("expected e=%+v, got=%+v", expected["a"], a)
	}
	f := secondLocal.Define("f")
	if f != expected["f"] {
		t.Errorf("expected f=%+v, got=%+v", expected["a"], a)
	}
}
...
func TestResolveFree(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if len(tt.table.FreeSymbols) != len(tt.expectedFreeSymbol) {
			t.Errorf("wrong number of free symbols. got=%d, want=%d", len(tt.expectedFreeSymbol), len(tt.table.FreeSymbols))
			continue
		}
...
func TestResolveUnresolvableFree(t *testing.T) {
	...
	secondLocal.Define("e")
	secondLocal.Define("f")
	expected := []Symbol{ // from secondLocal perspective
		{Name: "a", Scope: GlobalScope, Index: 0},
		{Name: "c", Scope: FreeScope, Index: 0},
		{Name: "e", Scope: LocalScope, Index: 0},
		{Name: "f", Scope: LocalScope, Index: 1},
